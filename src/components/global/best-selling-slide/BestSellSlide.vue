...
<template>
    <div
        ...
        <div class="w-full px-2 mt-5 lg:mt-7 gap-4 gap-y-0 lg:gap-0">
            <swiper :slidesPerView="4" :spaceBetween="4" :grabCursor="true" :modules="modules">
                <swiper-slide class="h-auto w-auto ml-[20px]" v-for="dataSlide in dataSelling" :key="dataSlide.id">
                    <div>
                        <a class="flex min-w-[280px] mt-5" v-for="data in dataSlide.slide" :key="data.number" :href="data.href" target="_blank">
                            <div class="flex min-w-[280px]">
                                ...
                                <div class="grow flex flex-col justify-center relative border-b border-neutral200">
                                    <p class="text-xs text-neutral700 font-semibold ellipsis-2 overflow-hidden">{{data.title}}</p>
                                </div>
                            </div>
                        </a>
                    </div>
